...
function setup_fmt() {
  ...
  if [ ! -d "$FMT_DIR" ] ; then
    ...
    git clone https://gitee.com/huifeng_github/fmt.git  "$FMT_DIR"
  fi
  cd "$FMT_DIR"
  git fetch --tags
...
function setup_googletest() {
  ...
  if [ ! -d "$GTEST_DIR" ] ; then
    ...
    git clone https://gitee.com/huifeng_github/googletest.git  "$GTEST_DIR"
  fi
  cd "$GTEST_DIR"
  git fetch --tags
...
function setup_zstd() {
  ...
  if [ ! -d "$ZSTD_DIR" ] ; then
    ...
    git clone https://gitee.com/huifeng_github/zstd.git "$ZSTD_DIR"
  fi
  cd "$ZSTD_DIR"
  git fetch --tags
...
function setup_folly() {
  ...
  if [ ! -d "$FOLLY_DIR" ] ; then
    ...
    git clone https://gitee.com/huifeng_github/folly.git "$FOLLY_DIR"
...
function setup_fizz() {
  ...
  if [ ! -d "$FIZZ_DIR" ] ; then
    ...
    git clone https://gitee.com/huifeng_github/fizz.git "$FIZZ_DIR"
  fi
